...
<?php } ?>
	...
	if(erore == 1) { alert(eror_text); code_el.focus(); return false; }
        ...
        <div class="field_4 tit">
        <div class="title_box">
        <label for="title"><?php echo __('Title'); ?></label>
        <div id="titlediv"> <input type="text" name="frontend_label" id="title" size="30" value="<?php echo $atr_data['frontend_label'] ?>" /> </div>
        </div>

     <div class="field_24 m_unit"><label for="frontend_unit"><?php echo __('Measurement unit'); ?></label> <input type="text" name="frontend_unit" id="frontend_unit" size="30" value="<?php echo $atr_data['frontend_unit'] ?>" /> <span>E.g., kilogram, meter</span> </div>        
        </div>
...
<?php $backend_input_arr = array('text', 'select', 'multiple-select', 'checkbox', 'textarea', 'date', 'map'); ?>
    ...
    <select name="is_used_for_sort_by" id="is_used_for_sort_by"<?php if ($_REQUEST['action'] == 'add' or !in_array($atr_data['backend_input'], array('text', 'select')) ) { ?> disabled="disabled"<?php } ?>>
       ...
       <div class="field_2"> <label for="is_visible_in_advanced_search"><?php echo __('Is used for advanced search'); ?></label>    
        ...
        <div class="field_2"> 
        <label for="is_historisable"><?php _e('Visibility in backend') ?></label>  
<?php $backend_vis_arr = array('all', 'simple', 'configurable', 'grouped', 'admin_only'); ?>
        <select name="is_historisable" id="is_historisable">
        <?php foreach ($backend_vis_arr as $vis) : ?>    
        	<option value="<?php echo $vis ?>" <?php if ($vis == $atr_data['is_historisable']) { ?>selected="selected"<?php } ?>><?php echo $vis ?></option>
		<?php endforeach; ?>
		</select>
        </div>    

<?php $placeholder_2 = __('Title 2');  if(($atr_data['backend_input'] == 'checkbox') or in_array($atr_data['code'], array('discount', 'special_price'))) { $placeholder_2 = __('Action products'); } ?>
        <div class="field_2"> 
        <label for="frontend_label_2"><?php _e('Title 2') ?></label> <input type="text" class="wide" name="frontend_label_2" id="frontend_label_2" size="80" value="<?php echo $atr_data['frontend_label_2'] ?>" placeholder="<?php echo $placeholder_2 ?>" /> 
     	</div>      
...
if(!$atr_data['backend_table']) { $atr_id_2 = $_REQUEST['id']; } // normal attribute options	
else { $atr_id_2 = $atr_data['backend_table']; } 
...
function qf9_opt_delete(option_id) {
	...
	el.parentNode.removeChild( el );
        ...
        <div class="field_4">
        <div class="title_box"> <h3><?php _e('Attribute options'); ?></h3> </div>
     <div class="field_24">  
<?php  
$atr_types_arr = array('select', 'multiple-select');  $atr_types = "('".implode("', '", $atr_types_arr)."')";	
$cur_atr_id = $_REQUEST['id'];
$table_atr = WOW_TABLE_ATTRIBUTE; 
$atr_query_45 = "SELECT $table_atr.id, $table_atr.code, $table_atr.frontend_label FROM $table_atr 				
		WHERE $table_atr.status = 'valid' AND $table_atr.id NOT IN ( $cur_atr_id ) AND $table_atr.backend_table IN ( '' ) AND $table_atr.backend_input IN $atr_types ORDER BY $table_atr.filter_position ASC"; 			
$attributes_arr_45 = $wpdb->get_results( $atr_query_45, ARRAY_A );
?>
		<div class="<?php if($atr_data['backend_table']) { ?> feat_field<?php } ?>">
        <label for="backend_table"><?php _e('Use options from another attribute'); ?></label> 
        <select name="backend_table" id="backend_table"> 
        	<option value=""> </option>
		<?php foreach ($attributes_arr_45 as $atr_1) : ?>
        	<option value="<?php echo $atr_1['id'] ?>" <?php if($atr_1['id'] == $atr_data['backend_table']) { ?>selected="selected"<?php } ?>><?php echo $atr_1['code'] ?></option>
		<?php endforeach; ?>
		</select>  
        </div>   
     </div>       
        ...
        <?php if(!$atr_data['backend_table']) : // normal attribute options ?>
...
<div class="options-header a_options"> <div class="colu name option_lab"><?php _e('Option label') ?></div> <?php if(strpos($atr_data['code'], 'color') !== false) { ?><div class="colu color_code"><?php _e('Color code') ?></div><?php } ?> <div class="colu position"><?php _e('Position') ?></div> </div>
        ...
        <input type="hidden" name="options[<?php echo $num ?>][id]" value="<?php echo $option['id'] ?>" />
            ...
            <span class="link4"><input type="text" class="class4" name="options[<?php echo $num ?>][link]" value="<?php echo $option['link'] ?>" placeholder="<?php _e('Option link') ?>" /> </span>
            <?php /*
            <span class="link5"><input type="text" class="opt_pos" name="options[<?php echo $num ?>][add_post_id]" value="<?php echo $option['add_post_id'] ?>" /> <span>(custom post id)</span> </span> 
              */ ?>
        	<div class="position">
        ...
        <?php else : // options from another attribute ?>
        <div class="field_2 options options_cust">
		<div class="options_list" > 
		 <?php foreach ($options_arr as $option) : ?>
        <div class="option-box" > 
        <input type="text" value="<?php echo apply_filters('the_title', $option['label']); ?>" readonly="readonly" />
        <span class="small-code"><?php echo $option['id'] ?></span>        	
        </div>
        <?php endforeach; ?>
       </div>
        </div>
        <?php endif; // __ options from another attribute ?>
...
<div class="note">
<br /><br />
<?php _e('If you want to use attribute as additional field on pages, the type must be set as "textarea" or "map".') ?> <br /><br />
