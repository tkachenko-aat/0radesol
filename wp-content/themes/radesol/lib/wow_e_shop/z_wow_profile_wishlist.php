...
$post_content = $_POST['description'];
	...
	$customer = $_POST['customer']; // ?? зараз не використовуються поля 'customer', а лише поля типу ['atrib']['email']; можливо, впровадити поля типу ['customer']['email'];
		$post_arr_2 = array();
...
$p_type = 'post';
if($_POST['p_type']) { $p_type = $_POST['p_type']; }
if($_POST['taxonomy']) {
	$taxo = $_POST['taxonomy'];
...
//////////////////// send email //////////////////////
$email_message_tit_2 = '<h3><a href="'.get_permalink($post_id_last).'">'.get_the_title($post_id_last).'</a></h3>';
$client_name = $_POST['atrib']['first_name'];
$email_message = __('New advertisement has been published by ').$client_name.'</br>'.$email_message_tit_2;
...
if($_POST['atrib']['email']) {
	$to_client = $_POST['atrib']['email'];	
	$email_message_2 = __('Your advertisement has been successfully published.').'</br>'.$email_message_tit_2;
	///// mail for customer //////
...
// foreach ($atrib_53_arr as $a_key) { update_post_meta($post_id, $a_key, ''); }
		...
		foreach ($atrib_arr as $a_key => $a_value) {
			$meta_key_1 = $a_key;
		if(is_array($a_value)) { /// 'multiple-select' 
			delete_post_meta($post_id, $meta_key_1);
			foreach ($a_value as $value_1) { 			
			add_post_meta($post_id, $meta_key_1, $value_1); 	
			}
		}		
		else { update_post_meta($post_id, $meta_key_1, $a_value); }
...
function vip_status_array() {
	$status_arr = array(
		// 'simple' => __('Simple'),		
		'highlight' => __('Highlight'),
		'vip' => __('VIP'),
		'urgently' => __('Urgently'),
		'top' => __('Raise the top'),
		'publish' => __('Publish')
	);
	return $status_arr;
}
