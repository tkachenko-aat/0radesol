...
function post_view_attributes($post_id) {
	...
	$atr_query = "SELECT DISTINCT $table_atr.id, $table_atr.code, $table_atr.backend_input, $table_atr.frontend_label, $table_atr.frontend_unit, $table_atr.sorting_position, $table_section.id AS group_id, $table_section.code AS group_code, $table_section.name AS group_name
				FROM $table_atr 			
	...
	foreach ($attributes_arr_2 as $key2 => $attribute) :
		$attrib_1 = array('id' => $attribute['id'], 'code' => $attribute['code'], 'frontend_label' => $attribute['frontend_label'], 'frontend_unit' => $attribute['frontend_unit'], 'backend_input' => $attribute['backend_input'], 'sorting_position' => $attribute['sorting_position']);
	...
	if($atr_values) {  //	
		...
		if($option_1['link']) { ////
                $opt_link = $option_1['link'];
				$target = '';  if(strpos($opt_link, 'http') === 0) { $target = ' target="_blank"'; }
				if(strpos($opt_link, 'http') !== 0 and $opt_link[0] != '#') { $opt_link = get_bloginfo('url').$opt_link; }				
                $atr_values[$key4] = '<a href="'.$opt_link.'"'.$target.'>'.$atr_values[$key4].'</a>'; 
        } // __ if($option_1['link'])
		} 
...
function post_view_my_group_attributes($post_id, $group_code) {
	...
	$attrib_arr = array();	
	$atr_query = "SELECT $table_atr.id, $table_atr.code, $table_atr.backend_input, $table_atr.frontend_label, $table_atr.frontend_unit, $table_atr.sorting_position, $table_atr.backend_table FROM $table_atr 				
				LEFT JOIN $table_details ON ($table_atr.id = $table_details.attribute_id )
				LEFT JOIN $table_section ON ($table_details.attribute_group_id = $table_section.id )
	...
	$atr_values_2 = $post_meta_arr[$attribute['code']];
	if($atr_values_2) { 	///	
	$atr_values = array();
		foreach ($atr_values_2 as $key4 => $m_value) {  
		$value_4 = '';
		if($attribute['backend_input'] == 'checkbox') { if($m_value == '') { $value_4 = __('No'); } else { $value_4 = __('Yes'); } }
		elseif ($m_value != '') {		
		$value_4 = $m_value;
		$option_1 = array(); ////
		if(in_array($attribute['backend_input'], array('select', 'multiple-select'))) { 
		$key4 = $m_value; // change key to option id 
		$option_1 = (array) $wpdb->get_row( "SELECT * FROM " . WOW_TABLE_ATTR_OPTIONS . " WHERE id = $m_value ", ARRAY_A );  
		$value_4 = $option_1['label']; 
			if($attribute['backend_table']) { // options from another attribute
		$atr_id_2 = $attribute['backend_table']; 
		$options_arr_2 = $wpdb->get_results( "SELECT * FROM " . WOW_TABLE_ATTR_OPTIONS . " WHERE attribute_id = $atr_id_2 ORDER BY position ASC ", ARRAY_A );	
		$options_arr = array();
			foreach ($options_arr_2 as $key7 => $opti) {	
		$options_arr[$opti['id']] = array( 'label' => apply_filters('the_title', $opti['label']), 'link' => apply_filters('the_title', $opti['link']) ); // $options_arr[$opti['id']] 
			}				
		$attributes_arr_2[$key2]['options'] = $options_arr;
			} // __ options from another attribute 
		} // __ 'select', 'multiple-select' 
		$atr_values[$key4] = apply_filters('the_title', $value_4); 
		if($option_1['link']) { ////
                $opt_link = $option_1['link'];
				$target = '';  if(strpos($opt_link, 'http') === 0) { $target = ' target="_blank"'; }
				if(strpos($opt_link, 'http') !== 0 and $opt_link[0] != '#') { $opt_link = get_bloginfo('url').$opt_link; }				
...
function post_view_one_attribute($post_id, $atr_code) {
	...
	$atr_query = "SELECT $table_atr.id, $table_atr.code, $table_atr.backend_input, $table_atr.frontend_label, $table_atr.frontend_unit, $table_atr.sorting_position FROM $table_atr 			
				LEFT JOIN $table_details ON ($table_atr.id = $table_details.attribute_id )
...
endforeach;	
	...
	$table_atr = WOW_TABLE_ATTRIBUTE; 
	$atr_query = "SELECT $table_atr.id, $table_atr.code, $table_atr.frontend_label, $table_atr.frontend_label_2, $table_atr.frontend_unit, $table_atr.is_visible_in_advanced_search FROM $table_atr WHERE $table_atr.code IN $conf_atrs_2 AND $table_atr.status = 'valid' ORDER BY $table_atr.filter_position ASC";
	$conf_attrib_arr = (array) $wpdb->get_results( $atr_query, ARRAY_A );

	foreach ($conf_attrib_arr as $key2 => $attribute) : 
	$conf_attrib_arr[$key2]['frontend_label'] = apply_filters('the_title', $attribute['frontend_label']);
	$conf_attrib_arr[$key2]['frontend_label_2'] = apply_filters('the_title', $attribute['frontend_label_2']);
	if($attribute['frontend_unit']) { $conf_attrib_arr[$key2]['frontend_unit'] = apply_filters('the_title', $attribute['frontend_unit']); } // Переклад
	
	$attrib_val_arr_k = $atr_meta_arr_25[$attribute['code']]; // масив значень даного атрибута ...
	$attrib_val_keyse = array();
	if(is_array($attrib_val_arr_k)) { $attrib_val_keyse = array_keys($attrib_val_arr_k); }
	...
	if(!count($conf_attrib_arr[$key2]['atr_options'])) { unset($conf_attrib_arr[$key2]); }	
			...
			if($conf_attrib_arr[$key2]['atr_options']) { // Переклад 
			foreach ($conf_attrib_arr[$key2]['atr_options'] as $key4 => $o_value) {
			$conf_attrib_arr[$key2]['atr_options'][$key4]['label'] = apply_filters('the_title', $o_value['label']);
			}
			}
...
$prev_atrs_8 = array();
$last_key4 = count($conf_attrib_arr) - 1;
foreach ($conf_attrib_arr as $key4 => $attribute) : 
if($key4 != $last_key4) { // всі атрибути, крім останнього 
	if($key4 != 0) { $prev_atrs_8[] = $conf_attrib_arr[$key4-1]['code']; }	
	$next_atr_code = $conf_attrib_arr[$key4+1]['code'];
	$atr_prod_arr_74[$attribute['code']]['next'] = $next_atr_code;
	if($key4 <= ($last_key4 - 2)) {  $next_atrs_82 = array(); 
for($key5 = $key4+2; $key5 <= $last_key4; $key5++) { $next_atrs_82[] = $conf_attrib_arr[$key5]['code']; }	 			
	$atr_prod_arr_74[$attribute['code']]['next-5'] = implode(',',$next_atrs_82); 
	}
	if(count($prev_atrs_8)) { $atr_prod_arr_74[$attribute['code']]['prev'] = implode(',',$prev_atrs_8); }
	$prod_atr_options = array();
	foreach ($attribute['atr_options'] as $option) : 
	foreach ($atr_prod_arr_7 as $key_7 => $prod_7) : 
	if(in_array($option['id'], $prod_7)) { /// 	
		$option_key_7 = $option['id'];
		if(count($prev_atrs_8)) { 
		$option_ids_8 = array();
		foreach ($prev_atrs_8 as $prev_atr_code) { $option_ids_8[] = $prod_7[$prev_atr_code]; } 
		$option_ids_8[] = $option['id'];
		$option_key_7 = implode('-', $option_ids_8);
		}		
  	$prod_opt_1 = $prod_7[$next_atr_code]; 
	$next_opt_ids_6 = $atr_options_ids_6[$key4+1];
	if(in_array($prod_opt_1, $next_opt_ids_6) ) { 
	// $atr_prod_arr_74[$attribute['code']]['next_opt_ids'] = $next_opt_ids_6;
	$prod_atr_options[$option_key_7][] = $prod_opt_1; 
	$prod_atr_options[$option_key_7] = array_values( array_intersect($next_opt_ids_6, array_unique($prod_atr_options[$option_key_7])) );	
	}   
	} // if(in_array($option['id'], $prod_7)) 
	endforeach; // ($atr_prod_arr_7 as $key_7 => $prod_7) 
	endforeach; 	
	$atr_prod_arr_74[$attribute['code']]['options'] = $prod_atr_options; 
} // if($key4 !=...)
endforeach;
...
function attributes_configur_filter($post_1st_id) {
		global $wpdb;
		global $wp_query;
		global $post;
		if (function_exists('qtrans_getSortedLanguages')) { global $q_config; } // -- Переклад		
	$table_atr = WOW_TABLE_ATTRIBUTE;  $table_details = WOW_TABLE_ATTRIBUTE_SECTION_DET;
	// $attributeSet_id = 1;
	$post_2_id = '';  if($post_1st_id) { $post_2_id = $post_1st_id; }
	$attributeSet_ids = WOW_Attributes_Front::get_attributeSet_id($post_2_id); // int or array 
	if(!is_array($attributeSet_ids)) { $attributeSet_ids = array($attributeSet_ids); }
	$set_ids = "(".implode(", ", $attributeSet_ids).")"; 
 $par_where_2 = "";
 if($post_1st_id or is_archive()) { $par_where_2 = " AND $table_details.attribute_set_id IN $set_ids"; }
	$atr_query = "SELECT $table_atr.id, $table_atr.code, $table_atr.backend_input, $table_atr.frontend_label, $table_atr.frontend_unit FROM $table_atr 			
				LEFT JOIN $table_details ON ($table_atr.id = $table_details.attribute_id )
				WHERE $table_atr.status = 'valid' AND $table_atr.is_visible_in_advanced_search = 'yes' AND $table_atr.is_configurable = 'yes'".$par_where_2."
				ORDER BY $table_atr.filter_position ASC";			
	$filt_attrib_arr = (array) $wpdb->get_results( $atr_query, ARRAY_A );
	$p_types = WOW_Attributes_Front::get_attrSet_posttypes();
	$p_types_2 = "('".implode("', '", $p_types)."')";
...
INNER JOIN $wpdb->postmeta ON ($wpdb->postmeta.meta_key = '$atr_code' AND $wpdb->postmeta.meta_value = $table_options.id)
INNER JOIN $wpdb->posts ON ($wpdb->postmeta.post_id = $wpdb->posts.ID)
	WHERE $table_options.attribute_id = $atr_id AND $wpdb->posts.post_type IN $p_types_2 AND $wpdb->posts.post_status = 'publish' 
	ORDER BY $table_options.position ASC ";
	$options_arr = (array) $wpdb->get_results( $options_query, ARRAY_A );
		if (count($options_arr)) { ////// //////////////////
	$filt_attrib_arr[$key2]['atr_options'] = $options_arr; ///	
...
function get_posts_args_by_atr_option($post_id, $atr_code_2, $p_type_2) {
global $wpdb;
	$meta_key_2 = $atr_code_2;  // $meta_key_2 = 'blog_author';  
	$table_atr = WOW_TABLE_ATTRIBUTE;  $table_atr_options = WOW_TABLE_ATTR_OPTIONS; 
	$option_1 = $wpdb->get_row( "SELECT $table_atr_options.*, $table_atr.frontend_label, $table_atr.frontend_label_2 FROM $table_atr_options 
	LEFT JOIN $table_atr ON ($table_atr_options.attribute_id = $table_atr.id)
	WHERE $table_atr_options.add_post_id = $post_id AND $table_atr.code = '$meta_key_2' ", ARRAY_A );
	$atr_option_id = $option_1['id']; 
	// $paged = (get_query_var('paged')) ? get_query_var('paged') : 1; // pagination
	$p_type = 'any';  if($p_type_2) { $p_type = $p_type_2; }
	$posts_args_2 = array(       
        'post_type'  => $p_type, // 'any', 'product'
		'posts_per_page'  => -1, // 6
		// 'paged' => $paged, // pagination
		'order' => 'ASC',	
		'orderby' => 'menu_order', // 'date', 'menu_order'	
		'meta_query' => array(
			array (
			'key' => $meta_key_2,
			'value' => $atr_option_id, // array(2, 5) // (for 'BETWEEN')
			// 'compare' => '=', // '=', '!=', '>', '>=', '<', '<=', 'IN', 'NOT IN', 'BETWEEN', 
			)
		),
		'post_status' => 'publish'
	);
$title_2 = apply_filters('the_title', $option_1['frontend_label_2']);
$posts_args_25 = array('title' => $title_2, 'posts_args' => $posts_args_2);

return $posts_args_25;
}


function product_get_unit($prod_id) {
	global $wpdb;
	if (function_exists('qtrans_getSortedLanguages')) { global $q_config; } // -- Переклад 
	$p_type = get_post_type($prod_id);
	$atr_set_data = $wpdb->get_row("SELECT * FROM " . WOW_TABLE_ATTRIBUTE_SET . " WHERE set_post_type = '$p_type'", ARRAY_A);
	$unit = $atr_set_data['unit'];
if($unit) {
		if (function_exists('qtrans_getSortedLanguages')) { // Переклад	
			$unit = qtrans_use($q_config['language'], $unit, true);
		} // -- Переклад
return $unit;
}
}
...
function product_get_price() {	
	global $post;
	$prod_id = $post->ID;
	$product_price_txt = '';  $old_price_txt = '';
	$main_prod_id = '';
	$clas_2 = '';
...
if($product_type == 'configurable') { $main_prod_id = WOW_Attributes_Front::configurable_prod_default(); }
	...
	if(get_post_meta($prod_id, 'special_price', true)) {  $clas_2 .= ' special-price';
		$old_price_2 = $price_2; 
		$price_2 = get_post_meta($prod_id, 'special_price', true); 
	}

	if($product_type == 'grouped') { 
$product_parts = get_post_meta($prod_id, 'grouped_product_parts', true);
...
$price_5_arr = array();
	foreach($product_parts_base_arr as $part_id) {  
	if(in_array($part_id, $product_parts_arr)) { $price_5_arr[] = get_post_meta($part_id, 'price', true); }
	}
$price_5 = array_sum($price_5_arr);
$price_2 = $price_2 + $price_5;  if($old_price_2) { $old_price_2 = $old_price_2 + $price_5; }
	}
	}

	$disc = WOW_Attributes_Front::product_get_discount($prod_id);
	if($disc) { $price_2 = $price_2 - ($disc/100)*$price_2;  $clas_2 .= ' discount'; }

	if($old_price_2) {
		$old_price = $old_price_2 * $kurs;  $old_price = round($old_price, $round_to);
		$old_price_txt = '<span class="old-price price">'.$old_price.'<span>'.$symb.'</span></span>';
	}
		
	$price = $price_2 * $kurs;  $price = round($price, $round_to);
	$product_price_txt = $old_price_txt.'<span class="price'.$clas_2.'">'.$price.'<span>'.$symb.'</span></span>';
...
function product_get_discount($prod_id) {
	$disc = 0;
...
function cart_row_subtotal($prod_id, $qty, $prod_parts_arr) {  /* Цінові правила .... */	
	if(strpos($prod_id, '__') !== false) { $id_4_2 = explode('__', $prod_id);  $prod_id = $id_4_2[0]; }
	$price_2 = 0;
	$meta_arr = get_post_custom($prod_id); 
	$price_2 = $meta_arr['price'][0];
...
if(count($price_arr_2)) { $qty_27 = max(array_keys($price_arr_2)); $price_2 = $price_arr_2[$qty_27]['price']; } /* !!!! */
		...
		if(count($prod_parts_arr)) { // grouped product 
			$part_price_5 = array();
			foreach ($prod_parts_arr as $part_id => $part_qty) { 
				$price_5 = get_post_meta($part_id, 'price', true);
				if(get_post_meta($part_id, 'special_price', true)) { $price_5 = get_post_meta($part_id, 'special_price', true); }
				$part_price_5[] = $price_5 * $part_qty;				
			}
			$part_price = array_sum($part_price_5);
			$price_2 = $price_2 + $part_price;
		} // if(count($prod_parts_arr))
...
function get_cart_discount() { 
	...
	$options_5 = get_option('wow_settings_arr');	
	if($options_5['wow_cart_discount_perc']) {  $discount_perc = $options_5['wow_cart_discount_perc'];
...
if(preg_match("/[^0-9.]/", $discount_perc) or (substr_count($discount_perc, '.') > 1)) { $discount_perc = 0; }	
	...
	$options_coupons = get_option('wow_coupons_list');
	if($options_coupons) { //// 
	$coupon_code = '';
	if(in_array('coupon', $po_arr)) { $coupon_code = $_POST['coupon']; }
	$coupons_arr_2 = array();  $coupon_act_key;
	foreach ($options_coupons as $key_2 => $coupon_1) { if($coupon_1['status'] == 1) {
		$coupons_arr_2[$key_2] = $coupon_1['code'];
		if($coupon_code and $coupon_1['code'] == $coupon_code) { $coupon_act_key = $key_2; }
	} }
	if(count($coupons_arr_2)) { 
	if($coupon_code and !in_array($coupon_code, $coupons_arr_2)) { $coupon_code = -1; }
	$discount_arr['coupon'] = $coupon_code;
		if($coupon_code and $coupon_code != -1) { 
	$discount_arr['coupon_label'] = $options_coupons[$coupon_act_key]['label'];
	$coupon_discount_perc = $options_coupons[$coupon_act_key]['discount']; 
	$coupon_discount_perc = str_replace(',', '.', $coupon_discount_perc); //
	if(preg_match("/[^0-9.]/", $coupon_discount_perc) or (substr_count($coupon_discount_perc, '.') > 1)) { $coupon_discount_perc = 0; } //
	$discount_perc = $discount_perc + $coupon_discount_perc; 
		}
	} } // if($options_coupons) { 
