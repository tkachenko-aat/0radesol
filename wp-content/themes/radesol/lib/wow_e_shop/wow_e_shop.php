...
function create_wow_menu() {
	...
	add_menu_page(__('WOW-shop'), __('WOW-shop'), 'edit_pages', WOW_ATTRIBUTE_LIST_URL, $attrib_display_func, '', 56);
		...
		if(E_SHOP_F == 1) : /* import - тільки для магазинів */
...
function create_wow_pages() {
	...
	$new_page_home = array( 'post_title' => 'Home title (change it)', 'post_name' => 'home', 'post_type' => 'page', 'menu_order' => 0, 'post_status' => 'publish');				
	...
	$pages_2_arr = array( 'about', 'about-1', 'about-2', 'about-3' );
	$menu_order = 3;
	foreach ($pages_2_arr as $name) { 
		$menu_order = $menu_order + 1;
	$parent = 0;  if($about_page_id) { $parent = $about_page_id; }
	$new_page_2 = array( 'post_title' => $name, 'post_name' => $name, 'post_type' => 'page', 'post_parent' => $parent, 'menu_order' => $menu_order, 'post_status' => 'publish');
	$page_2_id = wp_insert_post( $new_page_2, $wp_error );
	if($name == 'about') { $about_page_id = $page_2_id; }
	} // foreach ($pages_2_arr as $name)
		} // if(!get_option('page_on_front')) 
	...
	$new_page = array( 'post_title' => $name, 'post_name' => $name, 'post_type' => 'page', 'post_parent' => $parent, 'menu_order' => $menu_order, 'post_status' => 'publish');				
		...
		add_post_meta($page_id, '_aioseop_noindex', 'on', true); // all-in-one-seo-pack
		add_post_meta($page_id, '_aioseop_sitemap_exclude', 'on', true); // all-in-one-seo-pack 
	} // foreach ($pages_arr as $name => $page_template)
...
function write_wow_settings() {
	...
	$options_posts_page = array( 
		...
		'post_tag' => array('orderby' => 'date', 'order' => 'desc'),
...
if(E_SHOP_F != 0) : /// Session functions 
...
if(E_SHOP_F == 1) : /* !!!! // замовлення - тільки для магазинів */
include 'z_wow_checkout_order.php'; 
endif;
...
// if($opt_currency['symbols'][$base_currency]) { $b_symb = $opt_currency['symbols'][$base_currency]; }
	...
	// if ($price) { echo '<span class="price">'.$price.'<span class="symb"> '.$b_symb.'</span></span>'; }
	  $product_price = WOW_Attributes_Front::product_get_price();  echo $product_price;
	}
