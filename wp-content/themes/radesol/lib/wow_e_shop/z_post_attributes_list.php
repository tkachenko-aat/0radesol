...
/// Нові поля для сторінок. Використовуються атрибути типу textarea 
 $types_6_arr = array('page');  // array('page', 'post') 
 foreach ($types_6_arr as $p_type) {	
add_meta_box('wow_page_add_attributes', __('Page. New fields'), 'page_add_attributes', $p_type, 'normal', 'low');
...
$prod_type_arr = array('', 'all', $product_type);  
if( is_admin() ) { $prod_type_arr[] = 'admin_only'; }
$prod_type_2 = "('".implode("', '", $prod_type_arr)."')";
	...
	$par_where_prod_type = "";  $par_where_prod_type = " AND $table_atr.is_historisable IN $prod_type_2";
	$atr_query = "SELECT $table_atr.* FROM $table_atr 			
				LEFT JOIN $table_details ON ($table_atr.id = $table_details.attribute_id )
				WHERE $table_details.attribute_group_id = $group_id	AND $table_atr.status = 'valid'" . $par_where_prod_type . "
...
if(!$attribute['backend_table']) { $atr_id_2 = $attribute['id']; } // normal attribute options	
else { $atr_id_2 = $attribute['backend_table']; } // options from another attribute 
		$options_arr = $wpdb->get_results( "SELECT * FROM " . WOW_TABLE_ATTR_OPTIONS . " WHERE attribute_id = $atr_id_2 ORDER BY position ASC ", ARRAY_A );
...
$product_unit = WOW_Attributes_Front::product_get_unit($post->ID); 
?>

<div class="field_2 kiko2 atr-set">
<?php if($post_meta_arr['price'][0]) { 
$product_price = WOW_Attributes_Front::product_get_price(); 
?>
<span class="prod_prise"> <?php echo $product_price; ?> <?php if($product_unit) { echo '<span class="pr_unit"><span> / </span>'.$product_unit.'</span>'; } ?> </span>
<?php } ?>
<div class="atr_set"> <span><?php _e('Attribute Set') ?>: </span> <h4><?php echo $p_type_name; ?></h4> </div>
</div>
	...
	if(strpos($attribute['code'], 'price') !== false) { ///			
		$frontend_unit = $b_symb;
		if($product_unit) { $frontend_unit = $b_symb.'<span class="pr_unit"><span> / </span>'.$product_unit.'</span>'; }
		$attribute['frontend_unit'] = $frontend_unit;
	} ///	
	?>
    <li class="atr_item">
    <div class="field_2">
...
<?php } else { // ($attribute['backend_input'] == 'multiple-select' ) ?>
	...
	$option_item_id = $attribute['code'].'-option-'.$option['id'];
	?>
    <div class="line2">
    <input type="hidden" name="<?php echo $item_inp_key ?>" value="<?php echo $attribute['code'] ?>" />    
    <input type="checkbox" name="<?php echo $item_inp_name ?>" id="<?php echo $option_item_id ?>" value="<?php echo $option['id'] ?>" <?php if (in_array($option['id'], $sel_options2)) { ?>checked="checked"<?php } ?> /> <label class="cheki" for="<?php echo $option_item_id ?>"><?php echo $option['label'] ?></label>
    </div>
    <?php } ?>
    </div>
...
function post_prod_parameters() {
	...
	$stock_lab = __('Stock');
	$product_unit = WOW_Attributes_Front::product_get_unit($post_id);
	if($product_unit) { $stock_lab = $stock_lab. '<span class="unit"><span> , </span>'.$product_unit.'</span>'; }
	$system_pars_arr = array( 
		...
		'stock' => array('label' => $stock_lab, 'b_input' => 'text', 'clas' => ''),	
	...
	$prod_type = $post_meta_arr['product_type'][0]; // 'grouped' 
	if(!in_array($prod_type, array('configurable'))) { unset($system_pars_arr['configurable_ids']); }
...
if($prod_type == 'grouped') :  /// /// /// /// 

	echo '<div class="grouped_product">';
	
	$grouped_keys_5 = array('grouped_product_parts', 'grouped_product_parts_base');
	$grouped_pars_5 = array();
foreach ($grouped_keys_5 as $meta_key_5) :  /////
	add_post_meta($post->ID, $meta_key_5, '', true);
	$post_meta_arr_1 = $wpdb->get_row( "SELECT * FROM $wpdb->postmeta WHERE post_id = $post->ID AND meta_key = '$meta_key_5'", ARRAY_A );
	$meta_input_id = $post_meta_arr_1['meta_id']; 
	$input_value = $post_meta_arr_1['meta_value'];  $input_value_arr = explode(',', $input_value); 		
	$item_input_name = 'meta['.$meta_input_id.'][value]';
	$item_input_key = 'meta['.$meta_input_id.'][key]';
	$item_id = 'field_9_'.$meta_key_5;
		$grouped_pars_5[$meta_key_5]['value_arr'] = $input_value_arr;
		$grouped_pars_5[$meta_key_5]['item_id'] = $item_id;
	
	echo '<input type="hidden" name="'.$item_input_key.'" value="'.$meta_key_5.'" />'; ///
	echo '<input type="hidden" name="'.$item_input_name.'" id="'.$item_id.'" value="'.$input_value.'" />';
	// $item_input_name 
endforeach;
	
$main_post_type = get_post_type();
$parts_post_types = array (       
 $main_post_type => array('order' => 'ASC', 'orderby' => 'menu_order', 'title_1' => __('Product parts')),
 // 'prod_part2' => array('order' => 'ASC', 'orderby' => 'title', 'title_1' => __('Parts 2')),
);
	foreach ($parts_post_types as $p_type => $p_type_arr) :  /////
$posts_args_4 = array (       
        'post_type'  => $p_type,
		'order' => $p_type_arr['order'],	
		'orderby' => $p_type_arr['orderby'], // 'menu_order' 
		'posts_per_page' => -1,
		'meta_query' => array(
			array (
			'key' => 'product_type',
			'value' => 'simple', // ?? new type 'part' 
			)
		),
		'post_status' => array('publish', 'draft')
);
$parts_4_query = new WP_Query($posts_args_4);
if( $parts_4_query->have_posts() ) {
	echo '<div class="box_1 type-'.$p_type.'">';
	echo '<div class="tit" aria-label="1111 77"><h3>'.$p_type_arr['title_1'].'</h3></div>';	
	while ($parts_4_query->have_posts()) :  $parts_4_query->the_post(); ///
	$post_id = $post->ID;
	$part_inp_id = 'part-hxo-'.$post_id;
	$check = ''; if(in_array($post_id, $grouped_pars_5['grouped_product_parts']['value_arr'])) { $check = ' checked="checked"'; }
	$check_2 = ''; if(in_array($post_id, $grouped_pars_5['grouped_product_parts_base']['value_arr'])) { $check_2 = ' checked="checked"'; }
	$class_1 = 'check_prod_part_97';  $item_id_1 = $grouped_pars_5['grouped_product_parts']['item_id'];
	$onchange_f_1 = "set_grouped_product_parts_9("."'$class_1', '$item_id_1'".")";
	$class_2 = 'check_prod_part_b_97';  $item_id_2 = $grouped_pars_5['grouped_product_parts_base']['item_id'];
	$onchange_f_2 = "set_grouped_product_parts_9("."'$class_2', '$item_id_2'".")";
	echo '<span class="line_lab_2 prod_part">';
	echo '<label class="line_lab" for="'.$part_inp_id.'">'.get_the_title().'</label>  ';
	echo '<input type="checkbox" class="'.$class_1.'" id="'.$part_inp_id.'" name="part-'.$post_id.'" value="'.$post_id.'" onchange="'.$onchange_f_1.'"'.$check.' />';
	if($check) { echo '<span class="mak"><input type="checkbox" class="'.$class_2.'" name="part_base-'.$post_id.'" value="'.$post_id.'" title="'.__('Include as standard').'" onchange="'.$onchange_f_2.'"'.$check_2.' /></span>'; }
	echo '</span>';	
	endwhile; ///
	echo '</br></br> </div>';
...
?>
<script type="text/javascript">
function set_grouped_product_parts_9(check_class, item_inp_id) { 
	...
	var rez_item_id = document.getElementById(item_inp_id);
	rez_item_id.value = rez_text;
}
</script>
</div>
<?php endif; /// /// /// /// __ if($prod_type == 'grouped')  ?>
