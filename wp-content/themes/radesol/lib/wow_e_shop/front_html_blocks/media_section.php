...
$image_post_id = $post_id_gen;
if(wp_get_post_parent_id($post_id)) { $image_post_id = wp_get_post_parent_id($post_id); }
}
?>


	<?php if(has_post_thumbnail($image_post_id)) : ?> 
<?php 
...
if(in_array('popupp', $po_arr) and $gal_mode == 1) { $gal_mode = 0; }

$id_edic = 1;  if($gal_mode == 1) { $id_edic = $post_id; }
?>

<?php $thumbnail_id = get_post_thumbnail_id($image_post_id); 
...
<?php } elseif( $gallery_arr['slb_enab'] and !in_array('popupp', $po_arr) ) { ?> href="<?php echo $img_arr24[0] ?>" data-rel="lightbox-gallery-<?php echo $id_edic ?>" <?php } ?>
>
<?php echo get_the_post_thumbnail($image_post_id, $img_size); echo $gallery_arr['gallery_mode']; ?>
</a>
</div>
...
<?php endif; ?>
    ...
    <li>
    <a 
<?php if(is_single() and $gallery_arr['slb_enab'] and $gal_mode == 1) { ?>href="<?php echo $img_ss_full[0] ?>" data-rel="lightbox-gallery-<?php echo $id_edic ?>"
...
<?php if( is_single() and $gal_mode == 2 ) { // need for ajax load (in conf. product) ?>
<script type="text/javascript">
...
<?php } ?>
    ...
    <?php if (is_single() and $gal_mode == 0) : if($post->post_excerpt) { ?>
...
?>
	...
	<?php endif; // if has_post_thumbnail($image_post_id) ?>  
...
<?php /*  Додати вручну ефект lightbox 
<?php if( has_post_thumbnail() ) { ?> 
 <?php $thumbnail_id = get_post_thumbnail_id($post->ID); 
$img_arr24 = wp_get_attachment_image_src( $thumbnail_id, ''); ?>
 <a class="foto-icon" href="<?php echo $img_arr24[0] ?>" data-rel="lightbox-gallery-<?php echo $post->ID ?>"><?php the_post_thumbnail('thumbnail'); ?></a>
<?php } ?> 
 */ ?>
