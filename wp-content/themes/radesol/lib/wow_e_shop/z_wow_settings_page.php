...
$tabs_arr = array('wow_page' => __('General settings'));
if(E_SHOP_F == 1) : /* !!!! */
$tabs_arr = array('wow_page' => __('General settings'), 'pay_shipp' => __('Payment and shipping methods'), 'coupons' => __('Discount coupons'));
endif;
...
?>
        ...
        <?php
				...
				WOW_Settings::payment_shipping_methods_list();	
				} 
				
				if(in_array($cur_tab_key, array('coupons'))) {					
				WOW_Settings::payment_coupons_list();	
				} 
...
function wow_settings(){
	...
	add_settings_section(
		...
		if(E_SHOP_F == 1) : /* тільки для магазинів */
	add_settings_section(
 ...
 */		
	...
	add_settings_field(
		'wow_prod_configur_desc_1',    
		__('Conf product - desc_1'),   
		'wow_prod_configur_desc_1_input', 
		'wow_page',    
		'shop_labels'   
	);								

	add_settings_field(
		'wow_prod_configur_note_1',    
		__('Conf product - note_1'),   
		'wow_prod_configur_note_1_input', 
		'wow_page',    
		'shop_labels'   
	);								

	add_settings_field(
		'wow_prod_grouped_desc_1',    
		__('Grouped product - desc_1'),   
		'wow_prod_grouped_desc_1_input', 
		'wow_page',    
		'shop_labels'   
	);								

	add_settings_field(
		'wow_prod_grouped_note_1',    
		__('Grouped product - note_1'),   
		'wow_prod_grouped_note_1_input', 
		'wow_page',    
		'shop_labels'   
	);								
...
function wow_prod_configur_desc_1_input() {
	$options = get_option('wow_settings_arr');	$value = $options['wow_prod_configur_desc_1'];
	echo '<textarea id="wow_prod_configur_desc_1" name="wow_settings_arr[wow_prod_configur_desc_1]" placeholder="'.__('Please select product options').'" cols="120" rows="2">'.$value.'</textarea>';
}

function wow_prod_configur_note_1_input() {
	$options = get_option('wow_settings_arr');	$value = $options['wow_prod_configur_note_1'];
	echo '<textarea id="wow_prod_configur_note_1" name="wow_settings_arr[wow_prod_configur_note_1]" placeholder="'.__('Product options are not selected!').'" cols="120" rows="2">'.$value.'</textarea>';
}

function wow_prod_grouped_desc_1_input() {
	$options = get_option('wow_settings_arr');	$value = $options['wow_prod_grouped_desc_1'];
	echo '<textarea id="wow_prod_grouped_desc_1" name="wow_settings_arr[wow_prod_grouped_desc_1]" placeholder="'.__('Please select product components').'" cols="120" rows="2">'.$value.'</textarea>';
}

function wow_prod_grouped_note_1_input() {
	$options = get_option('wow_settings_arr');	$value = $options['wow_prod_grouped_note_1'];
	echo '<textarea id="wow_prod_grouped_note_1" name="wow_settings_arr[wow_prod_grouped_note_1]" placeholder="" cols="120" rows="2">'.$value.'</textarea>';
}
...
function payment_shipping_methods_list() {
	
	WOW_Settings::option_add_new_js();

$status_arr = array('1' => __('Enable'), '9' => '__'.__('Disable')); 
?>
...
?>    
	...
	<div id="<?php echo $input_name_1 ?>_list47" class="pay_list" >
    <?php 
	$options_pay = array();
	if(get_option($input_name_1)) { $options_pay = get_option($input_name_1); }
	$position_arr = array();
	...
	$num++;
	$option_id = $pref_1.'opt-'.$num;  // $pref_1 = 'pay_'; 
	?>
...
<?php if(in_array($method['code'], $online_methods_arr)) { ?><div class="test"><label class="line_lab" for="test_mode-<?php echo $num ?>"><?php _e('Test') ?></label> <input type="checkbox" id="test_mode-<?php echo $num ?>" name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][test_mode]" value="1"<?php if($method['test_mode'] == 1) { ?> checked="checked"<?php } ?> /></div><?php } ?>
    ...
    <div class="line_r"><a class="button button-primary" onclick="option_pp5_add_new('<?php echo $input_name_1 ?>', '<?php echo $pref_1 ?>')"><?php echo __('Add new method') ?></a> </div>
    </div> <!-- payment -->
...
<?php  
	...
	$pref_1 = 'shipp_';  // $pref_shipp = 'shipp_'; 
...
?>     
	...
	<div id="<?php echo $input_name_1 ?>_list47" class="shipp_list" >
    <?php 
	$options_shipp = array();
	if(get_option($input_name_1)) { $options_shipp = get_option($input_name_1); }
	$position_arr = array();
...
?>      
	...
	<div id="<?php echo $input_name_1 ?>_list47" class="cities_list" >
    <?php 
	$options_cities = array();
	if(get_option($input_name_1)) { $options_cities = get_option($input_name_1); }
	$position_arr = array();
    ...
    <div class="line_r"><a class="button button-primary" onclick="option_pp5_add_new('<?php echo $input_name_1 ?>', '<?php echo $pref_1 ?>')"><?php echo __('Add new city') ?></a> </div>
    </div> <!-- cities -->
...
function payment_coupons_list() {
	
	WOW_Settings::option_add_new_js();

$status_arr = array('1' => __('Enable'), '9' => '__'.__('Disable')); 
$type_price_arr = array('1' => __('% of subtotal'), '2' => __('Fixed price'), '3' => __('Other ...'));	
?> 
        <h3><?php _e('Coupons list') ?></h3>
<?php  
	$input_name_1 = 'wow_coupons_list';  
	$pref_1 = 'coupon_';  // $pref_coupon = 'coupon_';
?>    
    <div class="coupons field_2 options wide_opt">
	<script>
jQuery(document).ready(function($) {  
  ...
  changeYear: true 
    ...
    <div class="options-header"> <div class="colu name"><?php _e('Coupon name and code') ?></div> <div class="colu descr date"><?php _e('Date to') ?></div> <div class="colu type"><?php _e('Discount type') ?></div> <div class="colu type disc"><?php echo __('Discount').', % or price' ?></div> <div class="colu status"><?php _e('Status') ?></div> <div class="colu position"><?php _e('Position') ?></div> </div>
    
	<div id="<?php echo $input_name_1 ?>_list47" class="coupon_list" >
    <?php 
	$options_coupon = array();
	if(get_option($input_name_1)) { $options_coupon = get_option($input_name_1); }
	$position_arr = array();
	foreach ($options_coupon as $key_2 => $method) {
	$pos = 99; if($method['position'] != '') { $pos = $method['position']; } $position_arr[$key_2] = $pos; 
	}
	asort($position_arr);
	...
	foreach ($position_arr as $key_7 => $pos) :  //// ///// 
	$method = $options_coupon[$key_7];	
	$num++;
	$option_id = $pref_1.'opt-'.$num;  // $pref_1 = 'pay_'; 
	?>
	<div class="option-box" id="<?php echo $option_id ?>">
	<div class="colu name"><input type="text" name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][label]" value="<?php echo $method['label'] ?>" /> <div class="m_code"><span><?php _e('Code') ?>:</span><input type="text" name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][code]" value="<?php echo $method['code'] ?>" /></div></div>
    <div class="colu descr date"> <div class="m_code"><input type="text" class="datepicker" name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][date_to]" value="<?php echo $method['date_to'] ?>" readonly="readonly" /></div> </div>
    <div class="colu type"><select name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][type_price]">
    <?php foreach ($type_price_arr as $k_value => $label) { ?> <option value="<?php echo $k_value ?>" <?php if($k_value == $method['type_price']) { ?>selected="selected"<?php } ?>><?php echo $label ?></option> <?php } ?>
    </select></div>
    <div class="colu type disc"><input type="text" name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][discount]" class="w_50" value="<?php echo $method['discount'] ?>" /></div>
    <div class="colu status">
    <select name="<?php echo $input_name_1 ?>[<?php echo $pref_1.$num ?>][status]">
    <?php foreach ($status_arr as $k_value => $label) { ?> <option value="<?php echo $k_value ?>" <?php if($k_value == $method['status']) { ?>selected="selected"<?php } ?>><?php echo $label ?></option> <?php } ?>
    </select>
    ...
    <div class="line_r"><a class="button button-primary" onclick="option_pp5_add_new('<?php echo $input_name_1 ?>', '<?php echo $pref_1 ?>')"><?php echo __('Add new coupon') ?></a> </div>
    </div> <!-- 111 -->
