...
$r_total_arr = array();
$products_parts = array();  if($_POST['product_parts']) { $products_parts = $_POST['product_parts']; } 
...
$options_5 = get_option('wow_settings_arr');
  ...
  if($customer['city']) { $city_1 = explode('---', $customer['city']); $customer['city'] = $city_1[0]; }
	...
	/* subtotal - discount */ $subtotal_base = $subtotal_base - $disc_base;			
		...
		$post_arr_2['products_parts'] = $products_parts; /// 
		...
		if($disc_arr['coupon_label']) { $post_arr_2['coupon_label'] = $disc_arr['coupon_label']; } 
...
$products_parts = $excerpt_arr['products_parts']; ///
...
if($status == '') { unset($post_arr_24['status']); }
  ...
  foreach ($products_24 as $prod_id => $p_qty) { /* *** products *** */
			$prod_parts_arr = array();
			$prod_parts = '';  if(count($products_parts)) { if($products_parts[$prod_id]) {
			$prod_parts_arr = $products_parts[$prod_id];
			$prod_parts .= '<div class="parts">'; 
			foreach ($prod_parts_arr as $part_id => $part_qty) { 
				$prod_parts .= '<span>'.get_the_title($part_id).' <em>('.$part_qty.')</em></span>'; 
			}
			$prod_parts .= '</div>';
			} } // if(count($products_parts))
			$row_price_arr = WOW_Cart_Session::cart_get_row_price($prod_id, $p_qty, $prod_parts_arr);			
		...
		<td><span>'.$sku.'</span></td>
		<td> <h4><a href="'.get_permalink($prod_id).'" target="_blank">'.get_the_title($prod_id).'</a></h4>'.$prod_parts.'</td> 
...
function order_billing_info($post_id) {
	...
	if($excerpt_arr['coupon_label']) { $billing_arr['coupon_label'] = $excerpt_arr['coupon_label']; }
		} // if ( !empty($excerpt) )
...
function wow_order_billing_box() {
	...
	$output .= __('Shipping method').':  <span class="bolde">'.$billing_arr['shipp_label'].'</span></br></br>';
		if($billing_arr['coupon_label']) {
	$output .= __('Discount coupon').':  <span class="bolde">'.$billing_arr['coupon_label'].'</span></br></br>';
		}
...
function wow_order_products_box() {
		...
		foreach ($products as $prod_id => $p_qty) : 	  				
			...
			$row_price_arr = WOW_Cart_Session::cart_get_row_price($prod_id, $p_qty, $prod_parts_arr);
		$output .= '<li>
		<div class="colu prod_id"><span>'.$prod_id.'</span></div>
		<div class="colu prod_img"> <a title="'.get_the_title($prod_id).'">'.$thumb.'</a> </div>
		<div class="colu prod_sku"><span>'.$sku.'</span></div>
		<div class="colu prod_name"> <h4><a href="'.get_permalink($prod_id).'" target="_blank">'.get_the_title($prod_id).'</a></h4> '.$prod_parts.' </div> 		
