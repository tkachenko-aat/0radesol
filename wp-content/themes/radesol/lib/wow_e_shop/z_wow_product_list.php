...
function get_front_products_args($prod_type, $per_page_2) {
	$products_type_arr = WOW_Product_List_Func::get_front_products_type_arr();
	$args_2 = $products_type_arr[$prod_type]['args'];
	$p_types = WOW_Attributes_Front::get_attrSet_posttypes();  // array('product_1', 'product_2');	
	// if(in_array($prod_type, array('recomend_prod', 'popular_prod'))) { $p_types = WOW_Attributes_Front::get_attrSet_posttypes_no_prod(); }
	if($per_page_2) { $per_page = $per_page_2; }
	else {
	$tax_parameters_2 = WOW_Product_List_Func::get_tax_parameters($prod_type);
	...
	if(in_array($orderby, array('title', 'menu_order', 'date', 'modified', 'comment_count', 'ID', 'author', 'name'))) {
