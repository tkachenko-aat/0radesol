(function ( $ ) {
    ...
    var methods = {
        init : function(options) {
			...
			var percentage = $("<div class='percentage'><span class='progress-percentage'></span><em>%</em></div>");
    ...
    function insertText(scope) {
		...
		scope.find(".progress-percentage").text(Math.round(global_settings.percent)); /* !!!! */ 
    }
    // create canvas
    function createCanvas(scope) {
        ...
        if(canvas_height >= canvas_width) {
            radius = canvas_width * 1 / 2 - (global_settings.line_width * 1.4);
        } else {
            radius = canvas_height * 1 / 2 - (global_settings.line_width * 1.4);
        }
