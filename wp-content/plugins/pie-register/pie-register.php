...
class PieRegister extends PieReg_Base{
	...
	*/
	...
	function print_multi_lang_script_vars(){
		...
		'<?php echo __("Strength indicator"); ?>',
		'<?php echo (($options['pass_very_weak_label'])?__($options['pass_very_weak_label']):"");?>',
		'<?php echo (($options['pass_weak_label'])?__($options['pass_weak_label']):"");?>',
		'<?php echo (($options['pass_medium_label'])?__($options['pass_medium_label']):"");?>',
		'<?php echo (($options['pass_strong_label'])?__($options['pass_strong_label']):"");?>',
		'<?php echo (($options['pass_mismatch_label'])?__($options['pass_mismatch_label']):"");?>'
														 );
			
			var piereg_validation_engn = new Array(
												 '<?php _e("none");?>',
												 '<?php _e("Fill the field!");?>',
												 '<?php _e("* Please select an option");?>',
												 '<?php _e("* This checkbox is required");?>',
												 '<?php _e("* Both date range fields are required");?>',
												 '<?php _e("* Field must equal test");?>',
												 '<?php _e("* Invalid ");?>',
												 '<?php _e("Date Range");?>',
												 '<?php _e("Date Time Range");?>',
					'<?php _e("The minimum password length is 6 characters."); // "minSize" ?>',
												 '<?php _e(" characters required");?>',
												 '<?php _e("* Maximum ");?>',
												 '<?php _e(" characters allowed");?>',
												 '<?php _e("* You must fill one of the following fields");?>',
												 '<?php _e("* Minimum value is ");?>',
												 '<?php _e("* Date prior to ");?>',
												 '<?php _e("* Date past ");?>',
												 '<?php _e(" options allowed");?>',
												 '<?php _e("* Please select ");?>',
												 '<?php _e(" options");?>',
												 '<?php _e("Mismatch"); ?>',
												 '<?php _e("* Invalid credit card number");?>',
												 '<?php _e("* Invalid phone number");?>',
												 '<?php _e("* Allowed Format (xxx) xxx-xxxx");?>',
												 '<?php _e("* Minimum 10 Digits starting with Country Code");?>',
												 '<?php _e("* Invalid email address");?>',
												 '<?php _e("* Not a valid integer");?>',
												 '<?php _e("* Invalid number");?>',
												 '<?php _e("* Invalid month");?>',
												 '<?php _e("* Invalid day");?>',
												 '<?php _e("* Invalid year");?>',
												 '<?php _e("* Invalid file extension");?>',
												 '<?php _e("* Invalid date, must be in YYYY-MM-DD format");?>',
												 '<?php _e("* Invalid IP address");?>',
												 '<?php _e("* Invalid URL");?>',
												 '<?php _e("* Numbers only");?>',
												 '<?php _e("* Letters only");?>',
												 '<?php _e("* No special characters allowed");?>',
												 '<?php _e("* This user is already taken");?>',
												 '<?php _e("* Validating, please wait");?>',
												 '<?php _e("* This username is available");?>',
												 '<?php _e("* This user is already taken");?>',
												 '<?php _e("* Validating, please wait");?>',
												 '<?php _e("* This name is already taken");?>',
												 '<?php _e("* This name is available");?>',
												 '<?php _e("* Validating, please wait");?>',
												 '<?php _e("* This name is already taken");?>',
												 '<?php _e("* Please input HELLO");?>',
												 '<?php _e("* Invalid Date");?>',
												 '<?php _e("* Invalid Date or Date Format");?>',
												 '<?php _e("Expected Format: ");?>',
												 '<?php _e("mm/dd/yyyy hh:mm:ss AM|PM or ");?>',
												 '<?php _e("yyyy-mm-dd hh:mm:ss AM|PM");?>',
												 '<?php _e("* Invalid Username");?>',
												 '<?php _e("* Invalid File");?>',
												 '<?php _e("* Maximum value is ");?>'
												 );
		</script>
        <?php
...
if(in_array($post_name, array('login', 'registration', 'forgot-password'))) : 
...
/* !!!! */
		if(($pagenow == 'wp-login.php' && $option['block_wp_login']==1) && ($option['alternate_login']  && $theaction != 'logout'))  {
			...
			wp_safe_redirect('/404'); // wp_safe_redirect(get_permalink($option['after_login']));
			} else { wp_safe_redirect('/404'); } // wp_redirect(site_url());
		...
		/* !!!! */
		if(is_admin() && !current_user_can('publish_posts')) { wp_safe_redirect('/404'); }
	...
	function checkLogin()
	{
		...
		else
		{
			...
			if($error_found == 0){
				...
				if ( is_wp_error($user))
				{
		  $user_login_error = __('<strong>ERROR</strong>: Invalid username or incorrect password.');	
					/* 
				...
				else
				{
					...
					else
					{
						...
						if($active == "0")//If not active
						{
							...
							if($delete_user){
								wp_logout();
								$errors->add('login-error',apply_filters("piereg_your_account_is_not_activated",__('Your account is not activated yet.')));
							}
						}elseif(empty($active))
						{
							/*
	...
	{ 
		...
		$pie_page_suffix_1 = add_menu_page(__('Pie New Login' ), __('Pie New Login'), 'manage_options', 'pie-register', array($this,'RegPlusEditForm'), plugins_url("/images/pr_icon.png",__FILE__), 151);
		
		add_action('admin_print_scripts-' . $pie_page_suffix_1, array($this,'pieregister_admin_scripts_styles'));
			
			
		$pie_page_suffix_2 = add_submenu_page( 'pie-register', 'Form Editor', __('Form Editor'), 'manage_options', 'pie-register', array($this, 'RegPlusEditForm') );		
		
		add_action('admin_print_scripts-' . $pie_page_suffix_2, array($this,'pieregister_admin_scripts_styles'));	
		
			
		$pie_page_suffix_3 = add_submenu_page( 'pie-register', 'Pie-Settings', 'Pie. '.__('Settings'), 'manage_options', 'pie-general-settings', array($this, 'PieGeneralSettings') );
		/// 'pie-general-settings'	
...
var ajaxurl = '<?php echo admin_url('admin-ajax.php'); ?>';
	...
	function pie_save_registration()
	{
		...
		if(sizeof($errors->errors) == 0)
		{
			...
			/* !!!! */
			if(isset($_POST['url']))  { $user_data['user_url'] =  $_POST['url']; }
			...
			*/
			...
			if($option['enable_admin_notifications'] == "1")
			{
				...
				$to				= get_option('admin_email'); // trim($option['admin_sendto_email']);
				$from_name		= get_bloginfo('name'); // trim($option['admin_from_name']);
				$from_email		= $to; // trim($option['admin_from_email']);
				$bcc			= $to; // trim($option['admin_bcc_email']);
				$reply_to_email	= $to; // trim($option['admin_to_email']);
			...
			/* !!!! */
			if($option['verification'] == 0 ) { // no any verification
							if( $user ) {
								wp_set_current_user( $user->ID, $user->user_login );
								wp_set_auth_cookie( $user->ID );
								do_action( 'wp_login', $user->user_login );
