...
function pieResetFormOutput($piereg_widget = false){
	...
	$warning 	= __("Please enter your username or email address. You will receive a link to create a new password via email.");
	$success	= "";
	if(isset($_POST['user_login']) and trim($_POST['user_login']) == ""){
		$error[] = __('<strong>ERROR</strong>: Invalid username or e-mail.');
	}elseif (isset($_POST['reset_pass']))
	{
		...
		if($option['capthca_in_forgot_pass'] == 1){
			...
			if($privatekey){
				...
				if (!$resp->is_valid) {
					$error[] = '<strong>'.ucwords(__("error","piereg")).'</strong>: '.__('Invalid Security Code');
					$error_found++;
				}
			}
			...
			if(isset($_POST['piereg_math_captcha_forgot_pass']))
			{
				...
				if( ($cookie_result1 == $cookie_result2) && ($cookie_result3 == $_POST['piereg_math_captcha_forgot_pass'])){
					...
					$error[] = '<strong>'.ucwords(__("error")).'</strong>: '.__('Invalid Captcha');
					$error_found++;
				}
			}
		...
		if( $error_found == 0 ){
			...
			$error[] = __('<strong>ERROR</strong>: There is no user registered with that email address.');
			}
			/*
				*	If User Exist then
			*/
			...
			if ($user_exists){
				...
				if($allow){
				 ...
				 // Generate something random for a key...
					...
					if (FALSE == wp_mail($user_email, $subject, $message,$headers)){
						$error[] =  '<strong>'.ucwords(__("error")).'</strong>: '.__('The e-mail could not be sent.') . "<br />\n" . __('Possible reason: your host may have disabled the mail() function...') ;
					}
					...
					$error[] = apply_filters('piereg_password_reset_not_allowed_text',__("Password reset is not allowed for this user"));
				}
			   ...
			   $message .= network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . "&redirect_to=".urlencode(get_option('siteurl'))."\r\n";*/
				...
				if (count($error) == 0 )
				{
					$success =  apply_filters("piereg_message_will_be_sent_to_your_email", __('Check your e-mail for your new password.'));
	...
	$forgot_pass_form .= '
	  <form method="post" action="'.$_SERVER['REQUEST_URI'].'" id="piereg_lostpasswordform">
		...
		{
			$forgot_pass_form .= '<label for="user_login">'.((isset($option['forgot_pass_username_label']) && !empty($option['forgot_pass_username_label']))? __($option['forgot_pass_username_label']): __("Username or E-mail:")).'</label>';
		}
		  
    $forgot_pass_form .= '<input type="text" size="40" value="" class="input validate[required]" id="user_login" name="user_login" style="margin : 10px 0px;" placeholder="'.((isset($option['forgot_pass_username_placeholder']) && !empty($option['forgot_pass_username_placeholder']))? $option['forgot_pass_username_placeholder']: "").'">
		</p>
		...
		$forgot_pass_form .= '<p class="submit">';
		  ...
		  /// __('Reset Password') 
		  $forgot_pass_form .= '
		  <input type="submit" value="'.__('Get New Password').'" class="button button-primary button-large" id="wp-submit" name="user-submit">		  
		</p>';
		
		//if(!is_page()) {
		if(isset($pagenow) && $pagenow == 'wp-login.php' ){
			$forgot_pass_form .= '<p class="forgot_pass_links"> <a href="file://///192.168.14.2/projects/baqar/test_wp_plugin/wp-content/plugins/pie-register/'.wp_login_url().'">'.__('Log in').'</a> | <a href="file://///192.168.14.2/projects/baqar/test_wp_plugin/wp-content/plugins/pie-register/'.wp_registration_url().'">'.__('Register').'</a> </p>
			<p class="forgot_pass_links"><a title="'.__('Are you lost?').'" href="file://///192.168.14.2/projects/baqar/test_wp_plugin/wp-content/plugins/pie-register/'.get_bloginfo("url").'">&larr; '.__('Back to').' '.get_bloginfo("name").'</a></p>';
		}
		$forgot_pass_form .= '
