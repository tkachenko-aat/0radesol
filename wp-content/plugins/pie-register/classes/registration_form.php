...
function addUsername($form_widget = false) {
		...
		$data .= '<label for="username'.$formwidget.'">'.__("Login Name").'</label>';
		$data .= '<input id="username'.$formwidget.'" name="username" class="input_fields piereg_validate[required,username]" placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->getDefaultValue('username').'" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'"  />';	
return $data;
}
	function addPassword($fromwidget)
	{
		...
		$data .= 'id="'.$this->id.'" name="password" class="'.$this->addClass("input_fields",array("minSize[6]")).'" placeholder="'.$this->field['placeholder'].'" type="password" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" autocomplete="off" />';
	...
	function addEmail($fromwidget)
	{
		...
		if(isset($this->field['confirm_email']))
		{
			...
			//$widget = ( ($fromwidget)? ' pie_widget-2 #' : '' );
			$label2 = (isset($this->field['label2']) and !empty($this->field['label2']))? __($this->field['label2']) : __("Confirm E-mail");
			$data .= '</div></li><li class="fields8 pageFields_'.$this->pages.' '.$topclass .'"><div class="fieldset"><label>'.$label2.'</label><input  placeholder="'.$this->field['placeholder'].'" id="confirm_email_'.$this->id.'" '.$this->addValidation().' type="text" class="input_fields '.$this->field['css'].' piereg_validate[required,equals['.$this->id.']]" autocomplete="off">';
		}	
		return $data;
	}
	function addUpload()
	...
	{
		$data = "";
	$data .= '<div class="fieldset"><label for="'.$this->id.'_firstname">'.__("First Name").'</label>';
	$data .= '<input value="'.$this->getDefaultValue('first_name').'" id="'.$this->id.'_firstname" name="first_name" class="'.$this->addClass().' input_fields" '.$this->addValidation().'  type="text"  />';
		...
		// $label2 = (isset($this->field['label2']) and !empty($this->field['label2']))? __($this->field['label2']) : __("Last Name");
		/*$data .= '<div class="fieldset"><label>'.$label2.'</label>';*/
		$data .= '</div>';
		$data .= '<div class="fieldset fieldset_child"><label for="'.$this->id.'_lastname">'.__("Last Name").'</label>';
...
function addCheckRadio() {
		...
		if(sizeof($this->field['value']) > 0) {			
			...
			$field_class = 'fine_checkbox';
	/* !!!! url - radio // 'user_type' */ 
	if($this->field['type'] == 'radio') { $field_name = 'url';  $field_class = 'fine_radio'; }
	$user_types_arr = array( 'person' => __('Private person'), 'company' => __('Company') );
			
			$data .= '<div class="radio_wrap">';
			// $data .= $this->field['type'];
			$thispostedvalue = $this->getDefaultValue();
			for($a = 0 ; $a < sizeof($this->field['value']) ; $a++)  {
				
				$f_value = $this->field['value'][$a];
				$field_id = 'regi-'.$field_name.'-'.$a;
				$field_label = $this->field['display'][$a];
				if($user_types_arr[$f_value]) { $field_label = $user_types_arr[$f_value]; }
	...
	$data .= '<input '.$checked.' value="'.$f_value.'" type="'.$this->field['type'].'" '.((isset($multiple))?$multiple:"").' name="'.$field_name.'" class="'.$this->addClass("input_fields").' '.$field_class.'" id="'.$field_id.'" '.$this->addValidation().' data-map-field-by-class="'.$dymanic_class.'" >';				
					$data .= '<label for="'.$field_id.'">';
					$data .= $field_label;	
					$data .= '</label>';
					$data .= '</div>';
									//}
	...
	function addLabel()
	{
		...
		return '<label class="'.$topclass .'" for="'.$this->name.'">'.__($this->field['label']).'</label>';
	}
	...
	function addSubmit()
	{
		...
		{
			$data .= '<input name="pie_submit" type="submit" value="'.__($this->field['text']).'" />';
		}
		if($this->field['reset']==1)
		{
			$data .= '<input name="pie_reset" type="reset" value="'.__($this->field['reset_text']).'" />';
		}
		$data .= '</div>';
		return $data;
	}
	...
	function printFields($fromwidget = false)
	{
		...
		if(is_array($this->data)){
			foreach($this->data as $this->field)
			{
				...
				$pie_reg_fields .= '<li class="fields8 pageFields_'.$this->pages.' '.$topclass.'">';
				...
				switch($this->field['type']) :				
					...
					break;				
					case 'username' :
					/* !!!! */	$pie_reg_fields .= '<div class="fieldset">'.$this->addUsername($fromwidget).$this->addDesc().'</div>';
					break;
					case 'password' :
						$pie_reg_fields .= '<div class="fieldset">'.$this->addLabel();
				...
				*/
				...
				{		
					...
					$pie_reg_fields .=  '<div id="piereg_passwordStrength'.$widget.'" class="piereg_pass" >'.__("Strength indicator").'</div>';
					$pie_reg_fields .=  '</div>';
					$pie_reg_fields .=  '</li>';
				}
			}
