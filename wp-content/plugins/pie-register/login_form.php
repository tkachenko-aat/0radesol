			...
			elseif ( 'resetpass' == $_GET['action'] || 'rp' == $_GET['action'] ){
				...
				if ( is_wp_error($user) ) {
					if ( $user->get_error_code() === 'expired_key' )
						$login_error = '<strong>'.ucwords(__("ERROR:")).'</strong> '.apply_filters("piereg_you_key_has_been_expired",__("You key has been expired, please reset password again!").' <a href="'.pie_lostpassword_url().'" title="'.__("Password Lost and Found").'">'.__("Lost your password?").'</a>');
					else
						$login_error = '<strong>'.ucwords(__("ERROR:")).'</strong> '.apply_filters("piereg_this_reset_key_invalid_or_no_longer_exists",__("This Reset key is invalid or no longer exists. Please reset password again!").' <a href="'.pie_lostpassword_url().'" title="'.__("Password Lost and Found").'">'.__("Lost your password?").'</a>');
						$newpasspageLock = 1;
				}else{
					$login_warning = __('Enter your new password below.');
				}
				if(isset($_POST['pass1'])){
					$errors = new WP_Error();
					if(isset($_POST['pass1']) && trim($_POST['pass1']) == ""){
	  $login_error =  '<strong>'.ucwords(__("ERROR:")).'</strong>: '.apply_filters("piereg_invalid_password", __( 'The minimum password length is 6 characters.' )); // __( 'Invalid Password' )
						$errors->add( 'password_reset_mismatch',$login_error );
					}elseif ( isset($_POST['pass1']) and strlen($_POST['pass1']) < 6  ){
						$login_error =  '<strong>'.ucwords(__("ERROR:")).'</strong> '.apply_filters("piereg_minimum_8_characters_required_in_password", __( 'The minimum password length is 6 characters.' ));
						$errors->add( 'password_reset_mismatch',$login_error );
					}elseif ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ){
						$login_error =  '<strong>'.ucwords(__("ERROR:")).'</strong> '.apply_filters("piereg_the_passwords_do_not_match", __( 'Mismatch'));
						$errors->add( 'password_reset_mismatch',$login_error );
					}
					...
					if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
						...
						$login_warning = '';
						$login_error = '';
						$login_success = apply_filters("piereg_your_password_has_been_reset", __( 'Your password has been reset.' ));
					}
				}
			}
        }
...
if ( isset($_GET['action']) && ('rp' == $_GET['action'] || 'resetpass' == $_GET['action']) && ($newpasspageLock == 0) ){
	$form_data .= '
	  <form name="resetpassform" class="piereg_resetpassform piereg_loginform" action="'.pie_modify_custom_url(pie_login_url(),'action=resetpass&key=' . urlencode( $_GET['key'] ) . '&login=' . urlencode( $_GET['login'] )).'" method="post" autocomplete="off">
		...
		<div class="subm_line">
		  <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="'.__("Submit").'">
		</div>
		<div class="field">
		 <div class="nav">
		 	<a href="'.pie_login_url().'">'.__("Log in").'</a> | <a href="'.pie_registration_url().'">'.__("Register").'</a>
		 </div>
		</div>		
	  </form>';
}else{
	...
	if(isset($option['login_username_label']) && !empty($option['login_username_label'])){
			$form_data .= '<label for="user_login">'.__("Login Name").'</label>';
	}
	...
	if(isset($option['login_password_label']) && !empty($option['login_password_label'])){
		$form_data .= '<label for="user_pass">'.__("Password").'</label>';
	}
	...
	$form_data .= '
		...
		//if(!is_page()) {
			$form_data .= '<div class="subm_line">
			<p class="forgetmenot">
				<input type="checkbox" value="forever" id="p-rememberme" name="rememberme" />
				<label for="p-rememberme">'.__("Remember Me").'</label>
			</p>';
		//}
		$form_data .= '
		<p class="submit">
			<input type="submit" value="'.__("Log In").'" class="button button-primary button-large" id="wp-submit" name="wp-submit">
		...
		//if(!is_page() ) {
			$form_data .= '
			<p id="nav"> <a href="'.pie_registration_url().'">'.__("Register").'</a> <a class="separ" style="cursor:default;text-decoration:none;" href="javascript:;"> | </a> <a title="'.__("Password Lost and Found").'" href="'.pie_lostpassword_url().'">'.__("Lost your password?").'</a> </p>';
		//} ?>
	
		<?php if(isset($pagenow) && $pagenow == 'wp-login.php' ){
					$form_data .= '
					<p id="backtoblog"><a title="'.__("Are you lost?").'" href="'.bloginfo("url").'">&larr;'.__(" Back to").' '.get_bloginfo("name").'</a></p>';
			} 
	$form_data .= '
	</form>';
}
