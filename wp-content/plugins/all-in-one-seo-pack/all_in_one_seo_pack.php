...
Plugin URI: #
Description: Out-of-the-box SEO for your WordPress blog.  downloads since 2007.
Version: 2205.3.12
...
if ( ! defined( 'AIOSEOP_VERSION' ) ) {
	define( 'AIOSEOP_VERSION', '2205.3.12' );
}
...
/* 
add_action( 'init', 'aioseop_register_cat_meta' );
function aioseop_register_cat_meta() {
    register_meta( 'term', 'seo_title', array() );
}
 */
 
	/* !!!! */
add_action( 'init', 'aioseop_taxonomies_view_7' );

function aioseop_taxonomies_view_7() {
$taxo_arr = get_taxonomies(array('public' => true));
unset($taxo_arr['post_format']);
foreach ($taxo_arr as $taxo ) : 
add_action($taxo . '_edit_form_fields', 'aioseop_cat_fields');
// add_action($taxo . '_add_form_fields', 'aioseop_cat_fields');
add_action('edited_' . $taxo, 'aioseop_save_cat_fields', 10, 2);
// add_action('created_' . $taxo, 'aioseop_save_cat_fields', 10, 2);
endforeach;
}


function aioseop_cat_fields($term) {
    $term_id = $term->term_id;
	$seo_title = get_term_meta( $term_id, 'seo_title', true );
echo
    '<tr class="form-field jt-term-color-wrap">
        <th scope="row"><label for="cat-seo_title">'.__('Meta title').'</label></th>
        <td>
       <input type="text" name="seo_title" id="cat-seo_title" value="'.esc_attr($seo_title).'" class="seo_meta_field" />
        </td>
    </tr>';
}


function aioseop_save_cat_fields($term_id) {
...
function qaioseop_add_admin_page_config($page_configs) {
	global $pagenow;
$page_config = array();  $page_config['forms'] = array();
	if( in_array($pagenow, array('post.php', 'post-new.php')) ) { ////
// $page_config['pages'] = array('post.php' => '', 'post-new.php' => '');
$fields_arr_2 = array(
	'aioseop_snippet_title' => array( 'id' => 'aioseop_snippet_title', 'encode' => 'display' ),
	'aiosp_title' => array( 'tag' => 'input', 'name' => 'aiosp_title' ),
	'aiosp_description' => array( 'tag' => 'textarea', 'name' => 'aiosp_description' ),
	'aiosp_keywords' => array( 'tag' => 'input', 'name' => 'aiosp_keywords' ),
	'aiosp_custom_link' => array( 'tag' => 'input', 'name' => 'aiosp_custom_link' )
);
$page_config['forms']['post']['fields'] = $fields_arr_2;
	} elseif( in_array($pagenow, array('term.php', 'edit-tags.php')) ) { ////
// $page_config['pages'] = array('term.php' => '', 'edit-tags.php' => '');
$page_config['forms']['edittag']['fields']['seo_title'] = array('tag' => 'input', 'name' => 'seo_title');	
	}
