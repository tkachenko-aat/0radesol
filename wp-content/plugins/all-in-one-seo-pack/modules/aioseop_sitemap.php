	...
	class All_in_One_SEO_Pack_Sitemap extends All_in_One_SEO_Pack_Module {
		...
		function update_sitemap_from_posts( $new_status, $old_status, $post ) {
			...
			if ( ! empty( $this->options["{$this->prefix}posttypes"] ) ) {
				...
				if( !is_array($posttypes) ) { $posttypes = array('page', 'post'); } /* !!!! */
			}
			...
			foreach ( $files as $f ) {
				if ( $this->is_file( $f ) ) {
					...
					if ( ! empty( $file ) ) {
						...
						/* 
						if ( preg_match( '/<urlset ' . sprintf( $this->comment_string, '(.*)', '(.*)', '(.*)' ) . ' -->/',
							$file, $matches ) ) {
							if ( ! empty( $this->options["{$this->prefix}rewrite"] ) ) {
								$msg .= '<p>' . sprintf( __( "Warning: a static sitemap '%s' generated by All in One SEO Pack %s on %s already exists that may conflict with dynamic sitemap generation.", 'all-in-one-seo-pack' ),
										$f, $matches[2], $matches[3] ) . "</p>\n";
...
/* !!!! */
	...
	$home_links = array($home_url);
	if (function_exists('qtrans_getSortedLanguages')) {  global $q_config; /* Qtrans */	
$def_lang = $q_config['default_language']; 
foreach ( qtrans_getSortedLanguages() as $lang ) {
if($lang != $def_lang) {
		// $url = get_permalink($post);	
	$home_links[] = qtrans_convertURL($home_url, $lang, true, true); // ($url, $lang, false, true)
}
}
	}	
			$home = array(); /* !!!! */ 
			foreach ( $home_links as $url ) {
				$home[]           = array(
					'loc'        => $url, /// 
...
/* !!!! */
	$base_url = $this->get_term_link( $term, $term->taxonomy );
	$links = array($base_url);
	if (function_exists('qtrans_getSortedLanguages')) {  /* Qtrans */ 
$def_lang = $q_config['default_language']; 
foreach ( qtrans_getSortedLanguages() as $lang ) {
if($lang != $def_lang) {
	$links[] = qtrans_convertURL($base_url, $lang, true, true); // ($url, $lang, false, true)	
}
}
	} // __ Qtrans	
...
/* !!!! */ 
	$base_url = $url;
	$links = array($base_url);
	if ( $post->ID == get_option('page_on_front') ) { unset($links[0]); }
	else {
	if (function_exists('qtrans_getSortedLanguages')) {  /* Qtrans */ 
$p_content = $post->post_content;
$def_lang = $q_config['default_language']; 
if(strpos($p_content, '[:]') !== false and strpos($p_content, '[:'.$def_lang.']') === false) { unset($links[0]); }
foreach ( qtrans_getSortedLanguages() as $lang ) {
if($lang != $def_lang) {
	if(strpos($post->post_content, '[:'.$lang.']') !== false) { 
 		// $url = get_permalink($post);	
	$links[] = qtrans_convertURL($url, $lang, true, true); // ($url, $lang, false, true)
	}
}
}
	} // __ Qtrans	
